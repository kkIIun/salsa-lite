...
class Database:
    ...
    def __init__(self,
        ...
        self.max_nframes_per_file = int(30 * self.label_rate)  # hardcode the filelen of 60 seconds
    ...
    def get_split(self, split: str, split_meta_dir: str = '/meta/dcase2019', stage: str = 'fit'):
        """
    ...
    def load_chunk_data(self, split_filenames: List, split_sed_feature_dir: str, gt_meta_dir: str):
        ...
        for filename in split_filenames:
            ...
            feature_fn = os.path.join(split_sed_feature_dir, str(filename) + '.h5')
            ...
            # feature frames: n_frames is upsampled back to the original feature frame rat
            ...
            gt_meta_fn = os.path.join(gt_meta_dir, str(filename) + '.csv')
    ...
    def load_classwise_gt(self, gt_meta_fn, n_frames):
        ...
        if self.output_format in ['reg_xyz', 'accdoa']:
        ...
        elif self.output_format in ['azi']:
            sed_target = np.zeros((n_label_frames, self.n_classes), dtype=np.float32)
            doa_target = np.zeros((n_label_frames, self.n_classes), dtype=np.float32)
            ...
            for itrack in sorted_tracks:
                ...
                for idx, iframe in enumerate(frame_number_1):
                    class_idx = int(sound_class_idx_1[idx])
                    sed_target[iframe, class_idx] = 1.0
                    doa_target[iframe, class_idx] = azimuth_1[idx] / 180  # Radian unit
        elif self.output_format in ['reg_xy']:
            sed_target = np.zeros((n_label_frames, self.n_classes), dtype=np.float32)
            azi_target = np.zeros((n_label_frames, self.n_classes), dtype=np.float32)
            ...
            x[sed_target < 1] = 0.0
            y[sed_target < 1] = 0.0
            doa_target = np.concatenate((x, y), axis=-1)
        else:
