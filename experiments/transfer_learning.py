...
def train(exp_config: str = './configs/seld_salsa_lite.yml',
    ...
    # Resume training
    if resume:
        ckpt_list = [f for f in os.listdir(cfg.dir.model.best) if f.startswith('salsa-lite') and f.endswith('ckpt')]
        assert len(ckpt_list) >= 1, 'No checkpoint found'
        resume_from_checkpoint = os.path.join(cfg.dir.model.best, sorted(ckpt_list)[-1])
    ...
    # Tensorboard logger
    tb_logger = TensorBoardLogger(save_dir=cfg.dir.tb_dir, name='my_model', default_hp_metric=False)
    ...
    model.decoder.event_fc_2 = nn.Linear(64, 2, bias=True)
    model.decoder.azi_fc_2 = nn.Linear(64, 2, bias=True)
