...
class SeldDecoder(nn.Module):
    ...
    def __init__(self, n_output_channels, n_classes: int = 12, output_format: str = 'reg_xyz',
        ...
        # doa
        if self.doa_format == 'azi':
            self.azi_fc_1 = nn.Linear(self.fc_size, self.fc_size//2, bias=True)
            self.azi_dropout_1 = nn.Dropout(p=0.2)
            self.azi_fc_2 = nn.Linear(self.fc_size//2, self.n_classes, bias=True)
            self.azi_dropout_2 = nn.Dropout(p=0.2)

        elif self.doa_format == 'reg_xy':
    ...
    def init_weights(self):
        if self.doa_format == 'azi':
            ...
            init_layer(self.azi_fc_1)
            init_layer(self.azi_fc_2)
    ...
    def forward(self, x):
        ...
        # DOA: regression
        if self.doa_format == 'azi':
            azi_output = F.relu_(self.azi_fc_1(self.azi_dropout_1(x)))
            doa_output = torch.tanh(self.azi_fc_2(self.azi_dropout_2(azi_output)))
            ...
            doa_output = torch.cat((x_output, y_output), dim=-1)
